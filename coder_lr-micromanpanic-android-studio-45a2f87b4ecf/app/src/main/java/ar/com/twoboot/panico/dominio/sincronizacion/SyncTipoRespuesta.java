...
public class SyncTipoRespuesta extends Sincronizador {

	public SyncTipoRespuesta(Configuracion configuracion, Context context) {
		super(configuracion, context);
		setmNombreTabla("tipo_repuesta");
	...
	public boolean importar() {
		...
		try {
			resultado = true;
			String sqlQuery = "SELECT * FROM tipo_repuesta";
			...
			while (rs.next()) {
				valores.put("cod_tipo_respuesta", rs.getString(1));
				valores.put("tipo_respuesta", rs.getString(2));
